...
async fn process_one(
    ...
    let result = process_claimed(&processing, source.parent(), snapshot).await;
...
async fn process_claimed(
    ...
    #[cfg(unix)]
    {
        ...
        let request_dir = request_dir.ok_or_else(|| anyhow::anyhow!("request has no parent"))?;
        let directory_metadata = fs::symlink_metadata(request_dir)?;
        if !directory_metadata.file_type().is_dir()
            || directory_metadata.permissions().mode() & 0o022 != 0
        {
            anyhow::bail!("request directory must be a protected real directory");
        }
        if metadata.uid() != directory_metadata.uid() {
            anyhow::bail!("request owner does not match the request directory owner");
        }
...
fn validate_request_directory(directory: &Path) -> anyhow::Result<()> {
    ...
    #[cfg(unix)]
    {
        ...
        if metadata.permissions().mode() & 0o022 != 0 {
            anyhow::bail!("Headscale request directory must not be group/world-writable");
        }
    }
    Ok(())
}
...
fn write_snapshot(path: &Path, snapshot: &HeadscaleSnapshot) -> anyhow::Result<()> {
    let parent = path
        .parent()
        .ok_or_else(|| anyhow::anyhow!("Headscale state path has no parent"))?;
    let parent_metadata = fs::symlink_metadata(parent)?;
    if !parent_metadata.file_type().is_dir() {
        anyhow::bail!("Headscale state parent must be a real directory");
    }
    #[cfg(unix)]
    {
        use std::os::unix::fs::{MetadataExt, PermissionsExt};
        if parent_metadata.uid() != 0 || parent_metadata.permissions().mode() & 0o022 != 0 {
            anyhow::bail!(
                "Headscale state parent must be root-owned and not writable by group/other"
            );
        }
    }
    if let Ok(metadata) = fs::symlink_metadata(path) {
        if !metadata.file_type().is_file() {
            anyhow::bail!("Headscale state target must be a regular file");
        }
    }
    ...
    let temporary = parent.join(format!(".state-{suffix}.tmp"));
    let result = (|| -> anyhow::Result<()> {
        ...
        #[cfg(unix)]
        {
            ...
            chown(&temporary, None, Some(parent_metadata.gid()))?;
        }
